...
def send_ulog_file(file_path, server_url):
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            if response.status_code == 200:
                ...
                print(f"Failed to send file {file_path}: {response.status_code}")
                return False
...
/dev/ttyUSB0 or 0.0.0.0:14550. Auto-detect serial if not given.')
    ...
    # Отправка файла на сервер после завершения записи
    if os.path.exists(filename):
        if send_ulog_file(filename, args.server_url):
            print("Log file sent to server.")
        else:
            print("Failed to send log file to server.")
    else:
